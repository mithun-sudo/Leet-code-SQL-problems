...
chargebacked as (select
count(id) as chargebacks_count, 
sum(amount) as chargebacks_amount,
country,
extract('month' from charge_date) as month,
extract('year' from charge_date) as year
from chargebacks c
inner join transactions t on  c.trans_id = t.id
group by
country,
extract('month' from charge_date),
extract('year' from charge_date))
select
...
chargebacks_amount
from approved a
full outer join chargebacked c on a.month = c.month and a.year = c.year and a.country = c.country;
